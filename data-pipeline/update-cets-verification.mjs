...
const allowSourceMissingPdf = hasArg("--allow-source-missing-pdf");
...
const cdpEndpoint = readArg("--cdp-endpoint", process.env.CETS_CDP_ENDPOINT || "");
...
const stablePdfUrl = (value) => {
  ...
  try {
    ...
    url.search = "";
    url.hash = "";
    url.pathname = url.pathname.replace(/^\/cer\/ificate\.cets\.org\.cn\/cer\//, "/cer/");
    return url.href;
  } catch {
    return "";
  }
...
const fillMissingSiblingPdfUrls = (records) => {
  ...
  for (const rows of groups.values()) {
    // A single annual publication may deliberately expose one combined
    // attachment for several industry rows. Reuse it only within the same
    // year and only when every directly attached row points to the same PDF.
    const urls = [...new Set(rows.map((row) => row.pdf_url).filter(Boolean))];
    if (urls.length !== 1) continue;
    for (const row of rows) {
      if (row.pdf_url) continue;
      row.pdf_url = urls[0];
      row.pdf_filename = path.basename(new URL(urls[0]).pathname);
      row.pdf_url_source = "same_publication_group";
      filled += 1;
    }
  }
  return filled;
...
if (cdpEndpoint) {
  ...
  if (!context) throw new Error(`No browser context is available at ${cdpEndpoint}`);
  page = context.pages().find((candidate) => candidate.url().includes("cets.org.cn")) || await context.newPage();
} else if (profileDir) {
...
const ensurePublicPage = async () => {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      await page.waitForFunction(
        () => Boolean(globalThis.example1?.checkCount),
        null,
        { timeout: Math.min(challengeTimeoutMs, 60_000) },
      );
      lastNavigationError = null;
      break;
    } catch (error) {
      lastNavigationError = error;
      if (attempt < 3) await page.waitForTimeout(attempt * 1_500);
    }
  }
  if (interactive) {
  ...
  if (lastNavigationError) {
    ...
    throw new Error(`CETS public-information page did not become available within ${challengeTimeoutMs} ms: ${lastNavigationError.message}`);
  }
  ...
  if (cdpEndpoint) {
    await page.evaluate(() => {
      ...
      app.pageNo_3 = 1;
      app.pageSize_3 = Math.max(Number(app.checkCount) || 0, 2_000);
      app.getVerificationLits();
    });
    await page.waitForFunction(
      () => {
        const app = globalThis.example1;
        return Boolean(app) && !app.checkLoading && app.checkOrgLists?.length === Number(app.checkCount);
      },
      null,
      { timeout: 90_000 },
    );
...
try {
  ...
  const appCount = await page.evaluate(() => Number(globalThis.example1?.checkCount) || 0);
  const totalText = await page.locator(".el-pagination:visible").innerText().then(normalizeSpace).catch(() => "");
  const total = appCount || Number(totalText.match(/共\s*(\d+)\s*条/u)?.[1] || 0);
  if (!total) throw new Error("CETS verification list total is zero or unavailable");
  const pageSize = 40;
  const pageCount = Math.ceil(total / pageSize);
  const records = [];

  if (cdpEndpoint) {
    const rawRows = await page.evaluate(() => JSON.parse(JSON.stringify(globalThis.example1?.checkOrgLists || [])));
    const rowsToCapture = rawRows.slice(0, Math.min(rawRows.length, maxRecords));
    for (let rowIndex = 0; rowIndex < rowsToCapture.length; rowIndex += 1) {
      const rawRow = rowsToCapture[rowIndex];
      const province = normalizeSpace(rawRow.provinceName);
      const city = normalizeSpace(rawRow.cityName);
      const county = normalizeSpace(rawRow.countyName);
      const registeredAddress = [province, city, county].filter(Boolean).join("");
      const year = normalizeSpace(rawRow.year);
      const industry = normalizeSpace(rawRow.industryName);
      const institutionName = normalizeSpace(rawRow.orgName);
      const uscc = normalizeSpace(rawRow.creditCode);
      const authority = normalizeSpace(rawRow.deptName);
      const publishedAt = normalizeSpace(rawRow.createTime || rawRow.pubDate || rawRow.publicDate);
      const sourceKey = [year, registeredAddress, industry, institutionName, uscc, authority, publishedAt].join("|");
      const pdfUrl = pdfUrlFromObject(rawRow) || existingBySourceKey.get(sourceKey)?.pdf_url || "";
      const globalIndex = rowIndex + 1;
      records.push({
        verification_list_id: `VER-${String(globalIndex).padStart(4, "0")}`,
        source_record_id: rawRow.id ?? null,
        source_key: sourceKey,
        source_page: Math.ceil(globalIndex / pageSize),
        source_row_number: globalIndex,
        year,
        registered_address_raw: registeredAddress,
        province,
        city,
        county,
        region_parse_quality: province ? "source_fields" : "unparsed",
        industry,
    ...
    console.log(`captured ${records.length}/${Math.min(total, maxRecords)} from the public page state`);
  }

  for (let pageNo = 1; !cdpEndpoint && pageNo <= pageCount && records.length < maxRecords; pageNo += 1) {
  ...
  if (!probe) {
    ...
    const filledFromPublicationGroup = fillMissingSiblingPdfUrls(records);
    ...
    for (const row of missingLinks) row.pdf_url_source = "missing_at_source";
    if (missingLinks.length) {
      ...
      await fs.writeFile(manifestPath, JSON.stringify({
        ...
        source_url: sourceUrl,
        total,
        source_pdf_missing_count: missingLinks.length,
        records,
      }, null, 2), "utf8");
    }
    if (missingLinks.length) {
      ...
      await fs.mkdir(diagnostics, { recursive: true });
      await fs.writeFile(
        path.join(diagnostics, "missing-pdf-links.json"),
        JSON.stringify({ total, filledFromPublicationGroup, records: missingLinks }, null, 2),
        "utf8",
      );
      if (!allowSourceMissingPdf) {
        throw new Error(`${missingLinks.length} verification rows do not have a PDF URL after publication-group reconciliation`);
      }
    }
    ...
    if (!skipDownload) {
      ...
      const uniqueByFilename = new Map(records.filter((row) => row.pdf_url).map((row) => [row.pdf_filename, row]));
      const queue = [...uniqueByFilename.values()];
      const worker = async () => {
        while (queue.length) {
          ...
          if (lastError) {
            if (allowSourceMissingPdf && /HTTP (?:403|404|410)\b/.test(lastError.message)) {
              unavailableDownloads.push({
            ...
            throw new Error(`Failed to download ${record.pdf_url}: ${lastError.message}`);
          }
    ...
    console.log(JSON.stringify({
      ...
      uniquePdfs: new Set(records.map((row) => row.pdf_filename).filter(Boolean)).size,
      sourcePdfMissing: missingLinks.length,
      sourcePdfUnavailable: unavailableDownloads.length,
      downloaded,
      reused,
      filledFromPublicationGroup,
    }, null, 2));
    if (unavailableDownloads.length) {
      const diagnostics = path.join(workDir, "diagnostics");
      await fs.mkdir(diagnostics, { recursive: true });
      await fs.writeFile(
        path.join(diagnostics, "unavailable-pdf-downloads.json"),
        JSON.stringify(unavailableDownloads, null, 2),
        "utf8",
      );
    }
  }
